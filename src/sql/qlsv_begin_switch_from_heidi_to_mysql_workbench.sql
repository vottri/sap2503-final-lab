...
CREATE TABLE `employees` (
  ...
  `users_username` varchar(50) COLLATE utf8_unicode_ci NOT NULL,
  PRIMARY KEY (`code`,`users_username`),
  KEY `id` (`id`),
  KEY `fk_employees_users1_idx` (`users_username`),
  CONSTRAINT `fk_employees_users1` FOREIGN KEY (`users_username`) REFERENCES `users` (`username`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `results` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `course_code` varchar(50) COLLATE utf8_unicode_ci NOT NULL DEFAULT '0',
  ...
  KEY `id` (`id`),
  CONSTRAINT `FK_results_courses` FOREIGN KEY (`course_code`) REFERENCES `courses` (`code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `students` (
  ...
  KEY `id` (`id`),
  KEY `fk_students_users1_idx` (`users_username`),
  CONSTRAINT `fk_students_users1` FOREIGN KEY (`users_username`) REFERENCES `users` (`username`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
begin
	select count(*) into @cnt from users where username = _username and password = _password;
	if @cnt > 0 then
		select type into @utype from users where username = _username and password = _password;
		select username into @uname from users where username = _username and password = _password;
		select 'SUCCESS' as result, @utype as user_type, @uname as user_username;
	else
		select 'FAILURE' as result, 'tai khoan hoac mat khat khong dung' as msg;
	end if;
end ;;
DELIMITER ;
...
/*!50003 DROP PROCEDURE IF EXISTS `CreateResult` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreateResult`(
	IN `_code` varchar(50)
									  ,
	IN `_sub_chuyencan` varchar(50)
									  ,
	IN `_sub_giuaky` varchar(50)
									  ,
	IN `_sub_baitap` varchar(50)
									  ,
	IN `_sub_cuoiky` varchar(50)

)
begin
	insert into results(code, chuyencan, giuaky, baitap, cuoiky)
	values (_code, _sub_chuyencan, _sub_giuaky, _sub_baitap, _sub_cuoiky);
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreateStudent`(
	IN `_studentcode` varchar(50)
	,
	IN `_studentname` varchar(50)
	,
	IN `_studentdob` varchar(50)
	,
	IN `_studentpob` varchar(50)
	,
	IN `_studentmajor` varchar(50)

)
begin
	declare cnt int;
	select count(*) into cnt from students where code = _studentcode;
	if cnt = 0 then
		insert into students(code, name, dob, pob, major) values (_studentcode, _studentname, _studentdob, _studentpob, _studentmajor);
...
begin
	...
	where code = _code;
	if @cnt = 0 then
		insert into subjects(code, name, num_credit, major)
		values (_code, _name, _num_credit, _major);
		select 'SUCCESS' as result;
	else
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreateUser`(IN _username varchar(50), IN _password varchar(55), IN _type varchar(50))
begin
	select count(*) into @cnt
    from users
    ...
    if @cnt = 0 then
		...
		select 'FAILURE! username da ton tai';
    end if;
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
begin
	select count(*) into @cnt from students where code = _student_code;
	if @cnt > 0 then
		delete from students where code = _student_code;
...
begin
	select c.code as sub_code, c.name as sub_name, c.time as sub_time, r.chuyencan as sub_chuyencan, r.giuaky as sub_giuaky, r.baitap as sub_baitap, r.cuoiky as sub_cuoiky
	from results r, courses c
	where student_code = _student_code;
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
begin
	select code, name, dob, pob, major
	from students
	order by id desc
	limit n;
end ;;
...
begin
	select code, name, dob, pob, major
	from students
	where concat(code, name, dob, pob, major) like concat('%', _keyword, '%');
end ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
begin
	...
	if @cnt > 0 then
		UPDATE students
		set name = _student_name, dob = _student_dob, pob = _student_pob, major = _student_major
		where code = _student_code;
		select 'SUCCESS' as result;
	else
		select 'FAILURE' as result;
	end if;
...
-- Dump completed on 2018-12-23 21:05:50
